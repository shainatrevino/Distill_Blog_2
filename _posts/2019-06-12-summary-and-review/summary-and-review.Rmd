...
In this final section, we want to use `pmap()` to build custom plots that are able to visualize data across multiple inputs from a list. The question that we are looking to answer is: *How does average depression levels differ for males vs females across ethnicity?* 
...
There are now 27 plots that visualize our question of interest across mental health and income levels by using the `glue` package. Note that the plots are saved as a list within the object "plots." Thus, to access the plots we would: 1. specify the object "plots," 2. access the list "plots" by using the $ operation, 3. enter the number of which plot we want to view.
...
```{r saving plots}
# create an object files that uses meaningful names for the plots. Specify an object for each variable that we will save by (i.e., income and mental_health)
files <- str_replace_all(tolower(plots$income), " ", "-")
files2 <- str_replace_all(tolower(plots$mental_health), " ", "-")
# create the plot folder using fs
fs::dir_create(here::here("plots"))
# use the glue function to apply the object files to each plot with the object names we created above (i.e., files and files2)
paths <- here::here("plots", glue("{files}{files2}.png"))
